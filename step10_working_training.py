...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ["DQN", "PPO"]:
            config.agent_type = sys.argv[1]
            print(f"Using agent type: {config.agent_type}")
    
    if len(sys.argv) > 2:
        config.total_timesteps = int(sys.argv[2])
        print(f"Using total timesteps: {config.total_timesteps}")
    
    # Setup logging
    logger = setup_logging(os.path.join(config.output_dir, config.logs_dir))
    
    logger.info("Starting Catanatron RL training pipeline")
    logger.info(f"Configuration: Agent={config.agent_type}, Timesteps={config.total_timesteps}")
